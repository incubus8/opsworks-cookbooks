...
node[:deploy].each do |application, deploy|
  ...
  Chef::Log.debug("Running for #{node[:sidekiq][application]}")
  if node[:sidekiq][application].present?
    ...
    Chef::Log.debug("Check variable #{deploy[:environment_variables]} / ${deploy[:user]}")
